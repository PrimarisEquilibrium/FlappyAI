...
class GroundManager:
    ...
    def create_ground(self):
        furthest_x = 0
        while furthest_x < SCREEN_WIDTH + ground_img.get_width():
            self.ground_array.append(Ground(furthest_x, SCREEN_HEIGHT - 100))
            furthest_x += ground_img.get_width()
    
    def update(self, dt):
        for i, ground in enumerate(self.ground_array):
            if ground.x + ground_img.get_width() < 0:
                self.ground_array.append(Ground(self.ground_array[-1].x + ground_img.get_width(), SCREEN_HEIGHT - 100))
